...
-(id)init
{
    if (self = [super init]) {
        ...
        _refreshToken = [[sessionInfo objectForKey:@"refresh_token"] copy];
...
-(void)stroeUserSessionInfo
{
    NSMutableDictionary* sessionInfo = [NSMutableDictionary dictionaryWithCapacity:6];
    [sessionInfo setObject:self.accessToken forKey:@"access_token"];
    [sessionInfo setObject:self.refreshToken forKey:@"refresh_token"];
